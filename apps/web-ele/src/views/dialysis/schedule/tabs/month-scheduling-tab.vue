...
//   try {
//     const params = {
//       Date: formatMonth(searchMonth.value),
...
function generateMockData(): ScheduleItem[] {
  ...
  const year = searchMonth.value.getFullYear();
  const month = searchMonth.value.getMonth();
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    for (let j = 0; j < count; j++) {
      ...
      list.push({
        id: `SCH${String(idx + 1).padStart(4, '0')}`,
        date: dateStr,
        bedNo: `${(i % 20) + 1}号床`,
        patientName: names[i],
...
function handleAdd() {
  ...
  resetForm();
  formData.date = formatMonth(searchMonth.value) + '-01';
...
async function handleDelete(row: ScheduleItem) {
  try {
    await ElMessageBox.confirm(
      `确定要删除患者"${row.patientName}"在${row.date}的排班吗？`,
      '提示',
...
<template>
  <div class="month-scheduling-tab">
    <!-- 顶部工具栏 -->
    <div class="toolbar">
    ...
    >
      ...
      <ElTableColumn prop="date" label="日期" width="110" align="center" />
